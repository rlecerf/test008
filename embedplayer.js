...
function init () {
	$('#video').on('embedplayer:statechange', function (event) {
		...
		$('#currenttime').text(event.currentTime);
...
//System pour les Feedback
function playvideo(debut , fin){
	$('#video').embedplayer('seek',debut);$('#video').embedplayer('play');
		var timerExtrait = setInterval(function(){ 
		if(parseFloat($("#currenttime").html()) > fin){
			$('#video').embedplayer('pause');
			clearInterval(timerExtrait);
		}
	 },10);
 };
 function playvideo2 (debut){
	$('#video').embedplayer('seek',debut);$('#video').embedplayer('play');
 }
